...
工厂方法实现了多个工厂分门别类的管理多个产品
'''

class AbstractSchool(object):
    '''抽象化的factory类，不直接生成产品，只是制订一些通用的规范'''
    name = ''
    addr = ''
    principal = ''
...
class BJSchool(AttributeError):
    ...
    def create_course(self,course_type):
        if course_type == 'py_ops':
            course=PythonCourse('python自动化开发',7,'面授',11000)
        elif course_type == 'linux':
            course=LinuxOPSCourse('linux运维课程',5,'面授',7000)
        return course
    def enroll(self,name,course):
...
sh_school.info()
c1=sh_school.create_course('py_ops')
c2=sh_school.create_course('linux')

sh_school.enroll('张三',c1)
sh_school.enroll('王五',c1)

bj_school.info()
c1=bj_school.create_course('linux')
bj_school.enroll('李四',c1)
